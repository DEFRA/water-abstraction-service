...
const syncSupportedSourcesUpdateJob = require('../../../../src/modules/billing/jobs/sync-supported-sources-update');
...
experiment('modules/billing/jobs/sync-supported-sources-update', () => {
  ...
  test('exports the expected job name', async () => {
    expect(syncSupportedSourcesUpdateJob.jobName).to.equal('billing.supported-sources-update.sync-from-csv');
  });

  experiment('.createMessage', () => {
    test('creates the expected message array', async () => {
      const message = syncSupportedSourcesUpdateJob.createMessage(
        tempInvoiceAccountId
      );
      expect(message[0]).to.equal('billing.supported-sources-update.sync-from-csv');
      expect(message[1]).to.equal({});
      expect(message[2].jobId).to.startWith('billing.supported-sources-update.sync-from-csv.20');
  ...
  experiment('.onComplete', () => {
    beforeEach(async () => {
      await syncSupportedSourcesUpdateJob.onComplete();
    });

    test('an info message is logged', async () => {
      expect(logger.info.calledWith('billing.supported-sources-update.sync-from-csv: Job has completed')).to.be.true();
    });
  });
  ...
  experiment('.onFailed', () => {
    ...
    beforeEach(async () => {
      await syncSupportedSourcesUpdateJob.onFailed({}, error);
    });

    test('an error message is logged', async () => {
      expect(logger.error.calledWith('billing.supported-sources-update.sync-from-csv: Job has failed', error)).to.be.true();
    });
  });
});
