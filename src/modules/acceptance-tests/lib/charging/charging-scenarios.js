...
const supplementaryBillRun = async (request) => {
  ...
  const annualBatch = await billRun.createBatchAndExecuteBillRun(request, regionId, 'annual', 2020, false);
  if (annualBatch.status === 'error') {
    // return 500 error
    return Boom.badImplementation(`Annual batch failed to create. Batch: ${annualBatch}`);
  }
