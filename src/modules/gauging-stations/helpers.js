...
is equal to or greater than ${data.hol_rate_level}${data.hol_rate_level_unit}.`
  } else if (schema === '/wr22/2.3') {
    const activity = data.purpose_type === 'Power production' ? 'power production' : 'turbine'
 ...
 ${data.purpose_type === 'Abstraction' ? 'abstraction' : activity} shall not cause the ${data.hof_type === 'Hands off flow'
? `flow immediately downstream of the said ${abstractionPoint} 
      to fall below that rate.`
: `water level at of the said ${abstractionPoint} to fall below that level.`} 
 This shall be known as the ${data.hof_type}.`
  } else if (schema === '/wr22/2.4') {
